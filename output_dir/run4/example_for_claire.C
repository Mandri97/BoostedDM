...
void example_for_claire()
{
    ...
    auto tfile = TFile::Open("MuonEvents.root", "read");
    ...
    auto canvas = new TCanvas("name", "title", 700, 500);

    // Set automatic scientific notation on axes with large numbers
    TGaxis::SetMaxDigits(3);

    histogram->SetTitle("Muon Energy");                 // Set a title for the plot
    histogram->GetYaxis()->SetTitle("Arbitrary units"); // Set the title for the Y axis
    histogram->GetXaxis()->SetTitle("Energy (MeV)");    // Set the title for the X axis
    histogram->GetYaxis()->SetRangeUser(0, 7100e3);      // Set the limits in the Y axis
    histogram->GetXaxis()->SetLimits(15,30);          // Set the limits in the X axis
    histogram->GetXaxis()->SetRangeUser(15,30);
    histogram->SetLineColor(kRed);                      // Set a color for the histogram's line. You can find the name of the colors and color schemes at https://root.cern.ch/doc/master/classTColor.html
